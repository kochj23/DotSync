...
class NovaAPIServer {
    ...
    func start() {
        do {
            ...
            listener?.stateUpdateHandler = { if case .ready = $0 { print("NovaAPI [DotSync]: port \(self.port)") } }
            listener?.start(queue: .main)
        } catch { print("NovaAPI [DotSync]: failed — \(error)") }
    }
    ...
    private func route(_ req: NovaRequest) async -> String {
        ...
        case ("GET", "/api/status"):
            return json(200, ["status": "running", "app": "DotSync", "version": "1.0", "port": "\(port)", "uptimeSeconds": Int(Date().timeIntervalSince(startTime))])
        case ("GET", "/api/ping"):
